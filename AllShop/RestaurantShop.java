...
public class RestaurantShop extends MomButton implements ShopStatus {
    private int level;
    private int money;
    ...
    public RestaurantShop() {
        super("AllShop/Restaurant/1", 184, 96);
        setLayout(null);
        reMoney();
    ...
    public void startThread() {
        thread = new Thread(() -> {
            try {
                while (true) {
                    if (level < 4) {
                        super.setNamePath("AllShop/Restaurant/1");
                    } else if (level >= 4 && level < 7) {
                        super.setNamePath("AllShop/Restaurant/2");
                    } else if (level >= 7) {
                        super.setNamePath("AllShop/Restaurant/3");
                    }
                    reMoney();
