...
public class StormFlyShop extends MomButton implements ShopStatus {
    private int level;
    private int money;
    ...
    public StormFlyShop() {
        super("AllShop/StormFly/1", 184, 96);
        setLayout(null);
        reMoney();
    ...
    public void startThread() {
        thread = new Thread(() -> {
            try {
                while (true) {
                    if (level < 4) {
                        super.setNamePath("AllShop/StormFly/1");
                    } else if (level >= 4 && level < 7) {
                        super.setNamePath("AllShop/StormFly/2");
                    } else if (level >= 7) {
                        super.setNamePath("AllShop/StormFly/3");
                    }
                    reMoney();
