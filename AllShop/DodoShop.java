...
public class DodoShop extends MomButton implements ShopStatus {
    private int level;
    private int money;
    ...
    public void startThread() {
        thread = new Thread(() -> {
            try {
                while (true) {
                    if (level < 4) {
                        super.setNamePath("AllShop/Dodo/1");
                    } else if (level >= 4 && level < 7) {
                        super.setNamePath("AllShop/Dodo/2");
                    } else if (level >= 7) {
                        super.setNamePath("AllShop/Dodo/3");
                    }
                    reMoney();
