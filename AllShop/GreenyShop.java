...
public class GreenyShop extends MomButton implements ShopStatus {
    private int level;
    private int money;
    ...
    public void startThread() {
        thread = new Thread(() -> {
            try {
                while (true) {
                    if (level < 4) {
                        super.setNamePath("AllShop/Greeny/1");
                    } else if (level >= 4 && level < 7) {
                        super.setNamePath("AllShop/Greeny/2");
                    } else if (level >= 7) {
                        super.setNamePath("AllShop/Greeny/3");
                    }
                    reMoney();
